...
const CusEdit = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:3030/contacts" + cusid).then((res) => {
            return res.json();
        }).then((resp) => {
            idchange(resp.id);
            firstnamechange(resp.firstname);
            mailchange(resp.mail);
            phonenumberchange(resp.phonenumber);
            addresschange(resp.addressnumber);
            citychange(resp.city);
            statechange(resp.state);
            countrychange(resp.country);
            postalcodechange(resp.postalcode);
        }).catch((err) => {
    ...
    e.preventDefault();
    const cusdata={id,firstname,mail,phonenumber,address,city,state,country,postalcode};
    

    fetch(" http://localhost:3000/contacts"+cusid,{
        method:"PUT",
                        ...
                        <div className="card-body">
                            ...
                            <div className="row">
                                ...
                                <div className="col-lg-12">
                                    <div className="form-group">
                                        <label>Email</label>
                                        <input value={mail} onChange={e=>mailchange(e.target.value)} className="form-control"></input>
                                    </div>
                                </div>
                                ...
                                <div className="col-lg-12">
                                    <div className="form-group">
                                        ...
                                        <input value={postalcode} onChange={e=>postalcodechange(e.target.value)} className="form-control"></input>
                                    </div>
                                </div>
